...
getUserPostsById: (userId, page = 0, size = 10) =>
  ...
  getUserPosts: (page = 0, size = 10) =>
    api.get('/user/user-posts', { params: { page, size } }),
  ...
  updatePost: (id, title, content) =>
    api.put(`/user/user-posts/${id}`, { title, content }),

  // DELETE /user/user-posts/:id
  deletePost: (id) =>
    api.delete(`/user/user-posts/${id}`),

  // ── Like ──────────────────────────────────────────────────
  // POST /user/posts/:id/like  (toggle)
  likePost: (id) =>
    api.post(`/user/posts/${id}/like`),

  // ── Comments ──────────────────────────────────────────────
  // POST /user/posts/:id/comments
  createComment: (postId, content) =>
    api.post(`/user/posts/${postId}/comments`, { content }),

  // DELETE /user/comments/:id
  deleteComment: (id) =>
    api.delete(`/user/comments/${id}`),

  // ── Follow ────────────────────────────────────────────────
  // POST /user/followers/:followerId  (toggle follow/unfollow)
  follow: (followerId) =>
    api.post(`/user/followers/${followerId}`),
  ...
  // GET /user/reports?page=&size=
  getMyReports: (page = 0, size = 10) =>
    api.get('/user/reports', { params: { page, size } }),

  // DELETE /user/reports/:id
  deleteReport: (id) =>
    api.delete(`/user/reports/${id}`),
};