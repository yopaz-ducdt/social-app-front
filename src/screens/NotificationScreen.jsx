...
const TypeIcon = ({ type }) => {
  const map = {
    ...
    follow: ['bg-emerald-100', <Text key="follow" style={{ fontSize: 13, color: '#059669' }}>+</Text>],
  };
  ...
  return (
    <View className={`h-7 w-7 items-center justify-center rounded-full border border-white ${config[0]}`}>
      {config[1]}
    </View>
  );
};
  ...
  return (
    <View className="h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-gray-200">
      {uri ? (
        ...
        <Text className="text-sm font-semibold text-gray-700">{label.slice(0, 1).toUpperCase()}</Text>
      ) : (
        <IconUser />
      )}
    </View>
...
const NotifItem = ({ item, onPress }) => (
  <TouchableOpacity
    ...
    <View className="flex-1">
      ...
      <Text className={`text-xs ${item.unread ? 'text-blue-500' : 'text-gray-400'}`}>{item.time}</Text>
    </View>
